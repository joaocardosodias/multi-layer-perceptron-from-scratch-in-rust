...
pub fn evaluate_apac(
    ...
    let num_threads = rayon::current_num_threads();
    let chunk_size = ((num_images + num_threads - 1) / num_threads).max(1);
    let out_dim = mlp.dims.last().unwrap().0;
    ...
    let (correct, total_loss): (usize, f32) = (0..num_images)
        ...
        .par_chunks(chunk_size)
        .map(|indices| {
            let mut cache = BatchCache::new(&mlp.dims, m);
            let mut batch_input = vec![0.0f32; m * 784];
            let mut rng = StdRng::seed_from_u64(77);
            let mut c = 0usize;
            let mut loss = 0.0f32;

            for &i in indices {
                let src = &images[i * 784..(i + 1) * 784];

                let mut geo_buf = [0.0f32; 784];
                for j in 0..m {
                    let angle: f32 = rng.gen_range(-10.0..=10.0);
                    let tx: f32    = rng.gen_range(-1.5..=1.5);
                    let ty: f32    = rng.gen_range(-1.5..=1.5);
                    augment_image(src, &mut geo_buf, angle, tx, ty);
                    elastic_distort(
                        &geo_buf,
                        &mut batch_input[j * 784..(j + 1) * 784],
                        38.0, 6.0, &mut rng,
                    );
                }
                mlp.forward_batch(&batch_input, &mut cache, m, false, &mut rng);

                let a_off = cache.a_offsets[mlp.dims.len()];
                let mut log_sum = vec![0.0f32; out_dim];
                for j in 0..m {
                    let off = j * out_dim;
                    let probs = &cache.activations[a_off + off..a_off + off + out_dim];
                    for k in 0..out_dim {
                        log_sum[k] += probs[k].max(1e-30).ln();
                    }
                }

                let pred = (0..out_dim)
                    .max_by(|&a, &b| log_sum[a].partial_cmp(&log_sum[b]).unwrap())
                    .unwrap();

                if pred == labels[i] { c += 1; }

                loss += -log_sum[labels[i]] / m as f32;
            }
