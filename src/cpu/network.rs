        ...
        for i in 0..self.dims.len() {
            ...
            #[cfg(not(target_arch = "x86_64"))]
            {
                for s in 0..bs {
                    let offset = s * rows;
                    for r in 0..rows {
                        unsafe { *z_ptr.add(offset + r) += *b_ptr.add(r); }
                    }
                }
            }
            ...
            if i == self.dims.len() - 1 {
                for s in 0..bs {
                    ...
                    for r in 0..rows {
                        ...
                        unsafe { *a_ptr.add(offset + r) = e; }
                        sum_exp += e;
                    }
                    let inv_sum = 1.0 / sum_exp;
                    for r in 0..rows {
                        unsafe { *a_ptr.add(offset + r) *= inv_sum; }
                    }
                }
            } else {
                #[cfg(target_arch = "x86_64")]
    ...
    pub fn backward_batch(
        ...
        #[cfg(not(target_arch = "x86_64"))]
        {
            for r in 0..out_dim {
                ...
                unsafe { *db_last_ptr.add(r) += sum * inv_bs; }
            }
        }
        ...
        for l in (0..num_layers - 1).rev() {
            ...
            #[cfg(not(target_arch = "x86_64"))]
            {
                for rr in 0..r {
                    ...
                    unsafe { *db_l_ptr.add(rr) += sum * inv_bs; }
                }
            }
        }
    }
